...
export default function App() {
    ...
    const fetchMembros = async () => {
        ...
        setError('Aviso: Não foi possível conectar ao servidor. Verifique se a API está online.');
        setMembros([]);
        setLoading(false);
    };
    ...
    // ==========================================
    // RENDERIZAÇÃO DA TELA PÚBLICA (VISITANTE)
    // ==========================================
    if (isPublicMode) {
        return (
            <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans flex flex-col items-center p-4 py-10">
                <div className="w-full max-w-3xl flex flex-col items-center mb-8">
                    <div className="w-12 h-12 border-2 border-amber-500 flex items-center justify-center rounded-sm mb-4">
                        <span className="font-bold text-amber-500 tracking-tighter text-xl">I||D</span>
                    </div>
                    <h1 className="text-2xl md:text-3xl font-bold text-white uppercase tracking-wide text-center">Comunidade Casa de Deus</h1>
                    <p className="text-neutral-400 mt-2 text-center">Ficha de Cadastro de Membresia</p>
                </div>

                {cadastroEnviado ? (
                    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8 max-w-lg w-full text-center space-y-4 animate-in fade-in zoom-in duration-500">
                        <div className="w-20 h-20 bg-emerald-500/10 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6">
                            <CheckCircle2 size={40} />
                        </div>
                        <h2 className="text-2xl font-bold text-white">Cadastro Enviado!</h2>
                        <p className="text-neutral-400">Seus dados foram registrados com sucesso no nosso sistema. Deus abençoe a sua vida!</p>
                        <p className="text-xs text-neutral-600 pt-8">Você já pode fechar esta tela.</p>
                    </div>
                ) : (
                    <div className="w-full max-w-4xl relative">
                        {loading && (
                            <div className="absolute inset-0 bg-neutral-950/80 z-10 flex flex-col items-center justify-center rounded-2xl">
                                <Loader2 size={40} className="animate-spin text-amber-500 mb-4" />
                                <p className="text-amber-500 font-medium">Enviando dados...</p>
                            </div>
                        )}
                        <FormularioMembro onSave={handleAddMembro} isPublic={true} />
                    </div>
                )}
            </div>
        );
    ...
    // ==========================================
    // RENDERIZAÇÃO DO SISTEMA PRIVADO (ADMIN)
    // ==========================================
    return (
        <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans flex overflow-hidden">
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 lg:p-10 relative">
                    {loading && (
                        <div className="absolute top-4 right-4 bg-amber-500 text-black px-3 py-1 rounded-full text-sm font-bold flex items-center gap-2 z-50">
                    ...
                    {activeTab === 'novo_membro' && <FormularioMembro onSave={handleAddMembro} isPublic={false} />}
                </div>
            </main>
        </div>
    );
...
function FormularioMembro({ onSave, isPublic }) {
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-8 bg-neutral-900 border border-neutral-800 rounded-2xl p-6 md:p-8">
                ...
                <div className="flex justify-end pt-4 border-t border-neutral-800">
                    <button type="submit" className="px-6 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-white font-medium flex items-center gap-2 shadow-lg transition-colors mt-4 w-full md:w-auto justify-center">
                        <CheckCircle2 size={18} /> {isPublic ? "Enviar Cadastro" : "Salvar Membro"}
                    </button>
                </div>
            </form>
        </div>
